...
4. 开放-封闭原则：软件实体（类、模块、函数等）应该可以扩展，而不可修改，即对于扩展是开放的，对于修改是封闭的。
   ...
   - 查明可能发生的变化所等待的时间越久，创建正确的抽象就越困难，所以说尽量不要写翔山代码，否则重构都很难扭转这代码是坨翔的事实。
...
26. 职责链模式：使多个对象都有机会处理请求，从而避免请求的发送者和接收者之间的耦合关系。将这个对象连成一条链，并沿着这条链传递该请求，直到有一个对象处理它为止。

    ​	责任链模式在web请求中很常见，比如当我们需要为客户端提供一个REST服务，服务端针对客户端的请求实现用户鉴权、业务调用、结果反馈流程，就可以使用责任链模式实现。
...
27. 中介者模式：又称为调停模式，是迪米特法则的典型应用，用一个中介对象来封装一系列的对象交互。中介者使各对象不需要显式的相互引用，从而使其耦合松散，而且可以独立的改变它们之间的交互。

    ​	中介者模式包含以下主要角色：

    - 抽象中介者：中介者接口，定义了注册同事对象方法和转发同事对象信息的方法。
    - 具体中介者：中介接口的实现类，定义了一个List老保存同事对象，协调各个同事角色之间的交互关系。
    - 抽象同事类：定义同事类的公共方法和功能。
    - 具体同事类：抽象同事类的实现着，在需要与其他同事对象交互的时候，通过中介者对象来完成。

28. 享元模式：运用共享技术有效的支持大量细粒度的对象。

    - 如果一个应用程序使用了大量的对象，而这些对象造成了很大的存储开销时应该考虑使用享元模式。
    - 对象的大多数状态可以外部状态，如果删除对象的外部状态，那么可以用相对较少的共享对象取代很多组对象，此时考虑使用享元模式。
...
29. 解释器模式：给定一个语言，定义它的文法的一种表示，并定义一个解释器，这个解释器使用该表示来解释语言中的句子。
    ...
    - 抽象表达式：定义解释器的接口，约定解释器所包含的操作。
    - 终结符表达式：抽象表达式的子类，用于定义语法中和终结符表达式有关的操作，语法中的每一个终结符都应该有一个与之对应的终结表达式。
    - 非终结符表达式：抽象表达式的子类，用于定义语法中和非终结符表达式有关的操作，语法中的每条规则都有一个非终结符表达式与之对应。
    - 环境：定义各个解释器需要的共享数据或者公共的功能。

30. 访问者模式：表示一个作用于某对象结构中的各元素的操作。它使你可以在不改变各元素的前提下定义作用于这些元素的新操作。

    - 访问者模式适用于数据结构相对稳定的系统。

    - 它把数据结构和作用于结构上的操作直接的耦合解脱开，使得操作集合可以相对自由的演化。

    - 访问者模式的缺点是增加新的数据结构变得困难了。

      访问者模式实现的关键是将作用与元素的操作分离出来封装成独立的类，包含以下主要角色：

      - 抽象访问者：定义了一个访问元素的接口，为每类元素都定义了一个访问操作，该操作中的参数对应被访问元素的数据类型。
      - 具体访问者：抽象访问者的实现类，实现类不同访问者访问到元素后具体的操作行为。
      - 抽象元素：元素的抽象表示，定义了访问该元素的入口方法，不同的访问者类型代表不同的访问者。
      - 具体元素：实现抽象元素定义的操作，并根据访问者的不同类型实现不同的业务逻辑。
